...
int			is_map_surrounded(t_gameinfo *game_info)
{
	int				x;
	int				y;
	char			**map;
	...
	while (map && map[y])
	{
		...
		while (map[y][x])
		{
			if (ft_strchr("0NSWE", map[y][x]) != 0)
			{
				...
				if (x >= 0 && map[y][x - 1] == '9')
					return (0);
				if (y < get_maplen(map) - 1 && map[y + 1][x] == '9')
					return (0);
				if (x < ft_strlen(map[y]) - 1 && map[y][x + 1] == '9')
					return (0);
				if (y == 0 || x == 0 || y == get_maplen(map) - 1 || x == ft_strlen(map[y]) - 1)
					return (0);
			}
			x++;
		}
...
int			is_all_elements_valid(t_gameinfo *game_info)
{
	int				x;
	int				y;
	char			**map;
...
int			is_only_one_player(t_gameinfo *game_info)
{
	int				x;
	int				y;
	int				player;
...
int			is_validate_map(t_gameinfo *game_info)
{
...
{
    player -> th += th;
    if( player -> th < 0 ) player -> th += _2PI;
    else if( player ->th > _2PI ) player->th -= _2PI;
}
...
int			main(int argc, char *argv[])
{
	...
	init_gameinfo(game_info);
	init_player(player);
	printf("--------------init gameInfo----------------\n");
	print_gameinfo(game_info);
	printf("___________________________________________\n");
	parse_game(game_info, fd);
	printf("--------------parsed gameInfo----------------\n");
	print_gameinfo(game_info);
	printf("___________________________________________\n");
	get_player_position(player, game_info);
	printf("--------------PLAYER----------------------\n");
	print_player(player);
	if(is_validate_map(game_info) == 0)
	{
		printf("Error\n");
		exit(0);
	};
	...
	mlx_hook(window, X_EVENT_KEY_PRESS, 0, &key_press, player);

	load_image();
	/* IMAGE */
	
	image.image_ptr = mlx_new_image(mlx, game_info -> window_w, game_info -> window_h);
	image.data = (int*)mlx_get_data_addr(image.image_ptr, &image.bpp, &image.size_l, &image.endian);
